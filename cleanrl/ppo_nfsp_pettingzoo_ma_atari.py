...
def parse_args():
    ...
    parser.add_argument("--anticipatory", type=float, default=0.1,
        help="NFSP anticipatory parameter. Choose BR action with this probability.")
    parser.add_argument("--reservoir-buffer-size", type=int, default=100000,
        help="size of reservoir buffer")
    parser.add_argument("--reservoir-start-size", type=int, default=100000,
        help="required env steps to start training average network")

    args = parser.parse_args()
    args.batch_size = int(args.num_envs * args.num_steps)
    args.minibatch_size = int(args.batch_size // args.num_minibatches)
    args.avg_update_freq = args.batch_size // args.num_envs
    # fmt: on
    return args
...
class AverageAgent(nn.Module):
    ...
    def get_action(self, x, action=None):
        ...
        if action is None:
            action = probs.sample()
        return action, probs.log_prob(action), logits
...
class ReservoirBuffer: # (ExperienceReplayBuffer):
    ...
    def sample(self, num_samples: int):
        ...
        obss, actions = [torch.stack(x).to(self.device) for x in zip(*minibatch)]
        return obss, actions
...
class ParallelReservoirBuffer(ReservoirBuffer):
    ...
    def store(self, obss, actions, dones):
        # dones is from obss, not next obss

        # if states is None:
        #     return

        not_done_idxs = (1-dones).nonzero().flatten().tolist()
        for i in not_done_idxs:
            obs = obss[i].to(self.store_device)
...
if __name__ == "__main__":
    ...
    avg_agent = AverageAgent(envs).to(device)
    optimizer = optim.Adam(agent.parameters(), lr=args.learning_rate, eps=1e-5)
    avg_optimizer = optim.Adam(agent.parameters(), lr=args.learning_rate, eps=1e-5)
    ...
    reservoir = ParallelReservoirBuffer(
        size=args.reservoir_buffer_size,
        device=device,
        store_device='cpu',
        compress=False
    )
    ...
    # NFSP specific
    br_modes = torch.rand(args.num_envs).to(device) < args.anticipatory  # tracks BR vs. avg. action choice


    def avg_modes() -> torch.Tensor:
        return ~br_modes

    def sample_episode_policy(dones: torch.Tensor):
        """Sample average/best_response policies"""
        best_response_sample = torch.rand(args.num_envs).to(device) < args.anticipatory
        return dones*best_response_sample + ~dones*br_modes

    def _should_train_avg():
        return (global_step > args.reservoir_start_size
                and global_step % args.avg_update_freq < args.num_envs
                and len(reservoir) >= args.batch_size)
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                br_action, br_logprob, _, value = agent.get_action_and_value(next_obs)
                avg_action, avg_logprob, _ = avg_agent.get_action(next_obs)
                values[step] = value.flatten()
            action = br_modes*br_action + avg_modes()*avg_action  # join BR & avg actions
            logprob = br_modes*br_logprob + avg_modes()*avg_action
            ...
            if br_modes.any():
                # get best response observations/actions and store in reservoir buffer
                br_obs = next_obs[br_modes]
                br_action = action[br_modes]
                br_dones = next_done[br_modes]
                reservoir.store(br_obs, br_action, br_dones)
            ...
            if next_done.any():
                br_modes = sample_episode_policy(next_done.bool())
        ...
        if _should_train_avg(): # train average network with cross entropy
            res_obss, res_actions = reservoir.sample(args.batch_size)
            one_hot = F.one_hot(res_actions, num_classes=num_actions)
            _, _, logits = avg_agent.get_action(res_obss)
            log_probs = F.log_softmax(logits, dim=-1)
            ce_loss = - (one_hot * log_probs).sum(dim=-1).mean()
            writer.add_scalar(f"losses/avg_network_loss", ce_loss.item(), global_step)
